...
function insertOrder(){
      var queryString= 'select o.OrderId, o.userId, l.productId, count(*) as c, group_concat(productId) as productInfo, group_concat(quantity) as quant  from OrderItems o join LineItems l on o.OrderId=l.orderId group by o.OrderId limit ?,?';
      var values=[c,l];
      userArray=[];
      conn.query(queryString,values, function (error, results) {
        if (error) {
          ...
          for(i=0;i<results.length;i++){
            //console.log("VALUE:",ret[results.length-1].productInfo,l);
            var resProduct = ret[i].productInfo.split(",");
            var resQuantity= ret[i].quant.split(",");
            i=0; var arr=[];

              var obj1={
                'productId': resProduct[i],
                'quantity': resQuantity[i]
              }
